...
export class MaterialsController {
  ...
  @ApiOperation({ summary: 'Obtener catálogo de materiales paginado y filtrado' })
  @ApiQuery({ name: 'search', required: false, description: 'Búsqueda por código o descripción' })
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Cantidad de registros por página (default: 20)' })
  @ApiResponse({ status: 200, description: 'Retorna lista de materiales y metadata.' })
  ...
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Sincronizar catálogo con el sistema de la empresa (Solo Admin/Supervisor)' })
  @ApiResponse({ status: 200, description: 'Sincronización finalizada con éxito.' })
  ...
  @Get('external-mock')
  @ApiOperation({ summary: 'Mock público que simula el sistema externo de inventario de la empresa (Para testing)' })
