...
### 2.6 Tabla de Conciliación: Columna "Por Instalar" en Números Positivos
* **Reemplazo de Discrepancia por "Por Instalar":** En la tabla del *Reporte de Conciliación de Carga de Ingeniería*, se renombró la columna *"Discrepancia +/-"* por *"Por Instalar"*.
* **Formato Siempre Positivo:** Se calcula la cantidad de material pendiente por instalar como valor absoluto/positivo (`Math.max(0, cotizado - instalado)`). Si el material no tiene piezas pendientes, se visualiza en `0` (verde/neutro); si aún restan piezas por colocar en obra, se resalta en rojo con el número positivo exacto pendiente, eliminando los signos negativos que generaban confusión.
